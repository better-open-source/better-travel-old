...
namespace better_travel.Console
{
    internal static class Program
    {
        private static async Task Main()
        {
            ...
            var tester = InstagramConfig.Hashtag.Select(VisitHashtagUrl(InstagramConfig, page)).ToList() ;
            await browser.CloseAsync();
        }

        private static async Task<List<List<PostInfo>>> VisitHashtagUrl(AccountInformation InstagramConfig ,string hashTag, Page page)
        {
            var AllInfo = new List<List<PostInfo>>();

         
                await page.WaitForTimeoutAsync(2500);
                var HashtagUrl = ($"{InstagramConfig.BaseUrl}/explore/tags/{hashTag}/?hl=en");
